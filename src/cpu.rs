...
pub enum AddressingMode {
    Immediate,      // The value is the next byte itself
    ZeroPage,       // Address in the first 256 bytes of RAM
    ZeroPage_X,     // Zero page + register_x
    ZeroPage_Y,     // Zero page + register_y
    Absolute,       // Full 16-bit address
    Absolute_X,     // Address + register_x
    Absolute_Y,     // Address + register_y
    Indirect_X,     // Pointer on zero page + register_x
    Indirect_Y,     // Pointer on zero page + register_y
...
pub trait Mem {
    ...
    // Assemble/disassemble a u16 in two bytes. The 6502 uses little-endian.
    // The low byte goes in the lowest address, the high byte in pos+1
...
impl<'a> CPU<'a> {
    ...
    pub fn get_absolute_address(&mut self, mode: &AddressingMode, addr: u16) -> (u16, bool) {
        match mode {
            ...
            AddressingMode::Indirect_X => {
                ...
                ((hi as u16) << 8 | (lo as u16), false) // // Builds the final address
            }
    ...
    fn inx(&mut self) {
        self.register_x = self.register_x.wrapping_add(1); // Prevents overflow
        self.update_zero_and_negative_flags(self.register_x);
    }
        ...
        F: FnMut(&mut CPU),
    {
        ...
        loop {
            ...
            match code {
                ...
                0x6c => {
                    let mem_address = self.mem_read_u16(self.program_counter);
                    // https://6502.org/tutorials/6502opcodes.html#JMP
            ...
            // If the instruction is not a jump, you have to advance len - 1 bytes
            // more to get to the next instruction.
