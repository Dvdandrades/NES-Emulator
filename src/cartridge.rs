const NES_TAG: [u8; 4] = [0x4E, 0x45, 0x53, 0x1A]; // String NES^Z used to recognize .NES files
const PRG_ROM_PAGE_SIZE: usize = 16384; // 0x4000 (code)
const CHR_ROM_PAGE_SIZE: usize = 8192; // 0x2000 (visual graphics)
...
pub struct Rom {
    pub prg_rom: Vec<u8>, // Number of 16KB ROM banks
    pub chr_rom: Vec<u8>, // Number of 8KB VROM banks
    pub mapper: u8,
...
impl Rom {
    pub fn new(raw: &[u8]) -> Result<Rom, String> {
        ...
        let mapper = (raw[7] & 0b1111_0000) | (raw[6] >> 4); // For upper bits of ROM Mapper Type (Byte 7) | Four lower bits of ROM Mapper Type (Byte 6)

        let ines_ver = (raw[7] >> 2) & 0b11; // Byte 7
        if ines_ver != 0 {
        ...
        let skip_trainer = raw[6] & 0b100 != 0; // 512-byte trainer at 0x7000-0x71FF (Byte 6)
